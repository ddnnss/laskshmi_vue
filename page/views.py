...
def about_us(request):
    aboutActive = 'nav-item-link__active'
    title = 'О НАС'
    show_tags = True
...
def contacts(request):
    contactsActive = 'nav-item-link__active'
    show_tags = True
...
def dostavka(request):
    dostavkaActive = 'nav-item-link__active'
    show_tags = True
...
def new(request):
    newActive = 'nav-item-link__active'
    ...
    data = request.GET
    print(request.GET)
    items_count = all_items.count() + not_present.count()
...
def actions(request):
    actionActive = 'nav-item-link__active'
    all_items = Item.objects.filter(is_active=True, discount__gt=0).order_by('-discount')
    # not_present = Item.objects.filter(is_new=True, is_active=True, is_present=False)
    data = request.GET
    print(request.GET)
    search = data.get('search')
    ...
    try:
        ...
        items = items_paginator.page(items_paginator.num_pages)
    show_tags = False
    return render(request, 'page/action.html', locals())
...
def index(request):
    indexActive='nav-item-link__active'
    show_tags = True
    ...
    collections = Collection.objects.filter(show_at_homepage=True, is_special=False)
    special_collections = Collection.objects.filter(is_special=True)
...
def stars(request, subcat_slug):
    try:
        ...
        all_stars = Star.objects.all()
        star = Star.objects.get(name_slug=subcat_slug)
        all_items = Item.objects.filter(star_id=star.id, is_active=True, is_present=True).order_by('-created_at')
        np_all_items = Item.objects.filter(star_id=star.id, is_active=True, is_present=False)
